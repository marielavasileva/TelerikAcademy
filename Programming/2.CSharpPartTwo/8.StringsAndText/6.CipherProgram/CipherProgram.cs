...
class EncodeDecodeString
{
    static void Main()
    {
        ...
        string resultEncoded = Encryption(str, chiper);
        Console.WriteLine(resultEncoded);

        string resultDecoded = Encryption(resultEncoded, chiper);
        Console.WriteLine(resultDecoded);
    }

    private static string Encryption(string str, string cihper)
    {
        StringBuilder encrypt = new StringBuilder();

        for (int i = 0; i < str.Length; i += cihper.Length)
        {
            for (int j = 0; j < cihper.Length; j++)
            {
                if (i + j > str.Length - 1)
                {
                    break;
                }
                char temp = (char)(str[i + j] ^ cihper[j]);
                encrypt.Append(temp);
            }
        }
